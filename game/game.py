...
class VideoGame:
    ...
    def __init__(
        ...
        """Initialize a new game with the"""
        """given window size and window title."""
        pygame.init()
...
class BounceDemo(VideoGame):
    ...
    def build_scene_graph(self):
        ...
        soundtrack = os.path.join(
            self._data_dir, '09 - Warm Rays, Good Waves.mp3'
        )
        credits_string = 'Sound Effects: Monkey.aiff and Boing.aiff \
            from Mac OS 7. Soundtrack: Jack Pearcy,\
             "Warm Rays, Good Waves", Deep Lake Records. \
                 Images: explosion1.gif from Pygame.'
        self._scene_graph = [
            BlinkingTitle(
                self._screen,
                self._title,
